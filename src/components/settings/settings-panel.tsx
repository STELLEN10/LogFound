...
const sections = [
  {
    title: "Appearance",
    detail: "Choose the environment that helps you think clearly.",
    Icon: Palette,
  },
  {
    title: "Keyboard shortcuts",
    detail: "Your workspace is designed to stay under your hands.",
    Icon: Keyboard,
  },
  {
    title: "Voice",
    detail: "Wake phrase, transcription, and spoken recommendations.",
    Icon: Mic,
  },
  {
    title: "Agents",
    detail: "Tune the perspectives that join your strategy room.",
    Icon: Users,
  },
  {
    title: "GitHub",
    detail: "Repository context, sync health, and engineering memory.",
    Icon: Github,
  },
  {
    title: "Notifications",
    detail: "Only the signals worth interrupting for.",
    Icon: Bell,
  },
  {
    title: "Privacy",
    detail: "Your founder memory stays in your workspace.",
    Icon: Lock,
  },
];
...
export function SettingsPanel() {
  ...
  useEffect(() => {
    setDemo(localStorage.getItem("logfound-demo") !== "false");
  }, []);
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-5 py-9 sm:px-8 lg:px-10 lg:py-12">
      <section className="animate-rise border-b border-border/70 pb-9">
        ...
        <p className="mt-4 max-w-2xl text-lg leading-8 text-muted-foreground">
          A calm operating system should fit the way you think, speak, decide,
      ...
      <section className="mt-8 animate-rise animation-delay-1 rounded-xl border border-border bg-card/55 p-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Each theme keeps Logfound’s identity while changing the room
              around your work.
            </p>
          </div>
          <span className="rounded-full border border-primary/25 bg-primary/[0.08] px-2.5 py-1 text-xs text-primary">
            {theme || "midnight"}
          </span>
        </div>
        <div className="mt-6 grid gap-3 sm:grid-cols-5">
          {themes.map((option) => (
            ...
            >
              <span
                className={cn("block size-5 rounded-full", option.swatch)}
              />
              <p className="mt-5 text-sm font-medium">{option.name}</p>
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="mt-6 animate-rise animation-delay-2 rounded-xl border border-primary/20 bg-primary/[0.05] p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Keep a complete founder workspace filled with realistic projects,
              decisions, activity, and learning for a confident first look.
            </p>
          </div>
          <button
            onClick={toggleDemo}
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-2">
        {sections.map(({ title, detail, Icon }) => (
          <article
            key={title}
            className="animate-rise animation-delay-3 rounded-xl border border-border bg-card/55 p-5 transition-all hover:-translate-y-0.5 hover:border-primary/30"
          >
            ...
            <button className="mt-5 flex items-center gap-1 text-sm text-primary hover:underline">
      ...
      <section className="mt-6 flex items-center justify-between rounded-xl border border-border bg-card/55 p-5">
        <div>
          <p className="flex items-center gap-2 text-sm font-medium">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Everything is synced locally and ready for your next session.
          </p>
        </div>
