...
function logDevelopmentProviderError(error: unknown) {
  ...
  const responseBody = parseDebugBody(details?.responseBody ?? details?.body ?? response?.body ?? response?.data ?? message);
  console.error(`[ai] ${provider} SDK error (development only)`, {
...
export function toSafeAiError(error: unknown) {
  logDevelopmentProviderError(error);
